...
namespace OutSystems.HubEdition.DatabaseProvider.MySQL.Platform.Session {
    ...
    internal class MySQLPlatformSessionService : IPlatformSessionService {
        ...
        public void DeleteExpiredSession(string sessionId) {
            ...
            var sql = @"delete from " + TempSessionsTableName + @" 
                        where SessionId =  " + P.P("id") + @" and Expires < now();
                        delete from " + TempExtVarsTableName + " where SessionId = " + P.P("id") + " and row_count() > 0";
        ...
        public void UnlockSessionBySubstring(int position, string subString) {
            using (var con = DatabaseServices.TransactionService.CreateConnection()) {
                var e = DatabaseServices.ExecutionService;
                var sqlUnlockSMS = "SMSUnlockSessionsWithSubstring";
                using (var cmd = e.CreateCommand(con, sqlUnlockSMS)) {
                    cmd.CommandTimeout = queryTimeout;
                    cmd.CommandType = CommandType.StoredProcedure;
                    e.CreateParameter(cmd, P.P("position"), DbType.Int32, position);
                    e.CreateParameter(cmd, P.P("matchValue"), DbType.String, subString);
                    cmd.ExecuteNonQuery();
                }
            }
        }
        ...
        //TODO: dvn check if aditional validation is needed for java on mysql
        public void InsertInitialized(string sessionId, byte[] data, TimeSpan timeout) {
            try {
                var sql = "TempInsertInitializedItem";
