...
namespace OutSystems.HubEdition.DatabaseProvider.Oracle.TransactionService {
    ...
    public class TransactionService: BaseTransactionService {
        ...
        private static void SetJavaTNSADMINProperty() {
        }

        static TransactionService() {
            SetJavaTNSADMINProperty();
        }
        ...
        public override IDbConnection CreateConnection(int numRetries) {
            IDbConnection conn = null;
            
            // retry mechanism for first query (ALTER session...)
            int retryNumber = 0;
            while (retryNumber++ <= numRetries) {
                ...
                conn = base.CreateConnection(numRetries);
                try {
                    ...
                    ReleasePooledConnections(connError.Message);

                    // exiting on last retry
