...
namespace OutSystems.HubEdition.DatabaseProvider.Oracle.Platform.Session {
    ...
    internal class PlatformSessionService : IPlatformSessionService {
        ...
        private static readonly int ORACLE_UNIQUE_CONSTRAIN_VIOLATION_CODE = 1;
        ...
        public void DeleteExpiredSession(string sessionId) {
            ...
            var sql = @"declare deleted number; "
                      + @"begin deleted := 0;"
                        + @"DELETE FROM " + TempSessionsTableName + " "
                        + @"WHERE SessionId = " + P.P("id") + " and Expires < SYS_EXTRACT_UTC(SYSTIMESTAMP) "
                        + @"returning count(*) into deleted; "
                        + @"if (deleted > 0) then "
                                + "DELETE FROM " + TempExtVarsTableName + " "
                                + "WHERE SessionId = " + P.P("id") + ";"
                        + "end if;"
                      + @"end;";
            ...
            try {
                using (var conn = DatabaseServices.TransactionService.CreateConnection()) {
                    ...
                    using (var cmd = e.CreateCommand(conn, sql)) {
                        cmd.CommandTimeout = queryTimeout;
                        e.CreateParameter(cmd, P.P("id"), DbType.StringFixedLength, sessionId);
                        cmd.ExecuteNonQuery();
                    }
                }
            } catch (Exception e) {
        ...
        public void UnlockSessionBySubstring(int position, string subString) {
            using (var con = DatabaseServices.TransactionService.CreateConnection()) {
                var e = DatabaseServices.ExecutionService;
                var sqlUnlockSMS = P.SP("BEGIN SMSUnlockSessionsWithSubstring({0}, {1}); END;", "position", "subString");
                using (var cmd = e.CreateCommand(con, sqlUnlockSMS)) {
                    cmd.CommandTimeout = queryTimeout;
                    e.CreateParameter(cmd, P.P("position"), DbType.Int32, position);
                    e.CreateParameter(cmd, P.P("subString"), DbType.String, subString);
                    cmd.ExecuteNonQuery();
                }
            }
        }
        ...
        public void InsertInitialized(string sessionId, byte[] data, TimeSpan timeout) {
            try {
                var sql = P.SP("BEGIN TempInsertInitializedItem({0}, {1}, {2}); END;", "id", "item", "timeout");
                ...
                if (e.ErrorCode == ORACLE_UNIQUE_CONSTRAIN_VIOLATION_CODE) {
        ...
        public void InsertOrUpdate(string sessionId, int lockId, byte[] data, TimeSpan timeout) {
            try {
                using (var con = DatabaseServices.TransactionService.CreateConnection()) {
                    var e = DatabaseServices.ExecutionService;
                    using (var cmd = e.CreateCommand(con, P.SP("BEGIN TempCreateOrUpdateStateItem({0}, {1}, {2}, {3}); END;", "id", "item", "timeout_", "lockCookie"))) {
                        cmd.CommandTimeout = queryTimeout;
                        cmd.CommandType = CommandType.Text;
                        e.CreateParameter(cmd, P.P("id"), DbType.StringFixedLength, sessionId).Size = SessionConstants.ID_LENGTH;
                        var p = e.CreateParameter(cmd, P.P("item"), DbType.Binary, data);
                        Params.SetBlobType(p);
                        e.CreateParameter(cmd, P.P("timeout_"), DbType.Int32, Convert.ToInt32(timeout.TotalMinutes));
                        e.CreateParameter(cmd, P.P("lockCookie"), DbType.Int32, lockId);
                        cmd.ExecuteNonQuery();
                    }
                }
