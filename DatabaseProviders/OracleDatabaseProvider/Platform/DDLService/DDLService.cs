...
namespace OutSystems.HubEdition.DatabaseProvider.Oracle.Platform.DDLService {
    public class DDLService : BaseDDLService {
        ...
        private string CreateAdditionalPrimaryKeyIndexForCIAI(string tableName, string primaryKeyConstraintName, 
                IPlatformTableSourceColumnInfo column) {

            IPlatformDMLIdentifiers dmlIdentifiers = DatabaseServices.DMLService.Identifiers;

            string idxName = primaryKeyConstraintName.Replace(PlatformDatabaseObjectConstants.PrimaryConstPrefix, PrimaryConstInvariantPrefix).Replace(
            ...
            return string.Format("CREATE UNIQUE INDEX {0} ON {1} (NLSSORT({2}, 'NLS_SORT=BINARY_AI'))",
                ...
                dmlIdentifiers.EscapeAndQualifyIdentifierForLocalDatabase(tableName), dmlIdentifiers.EscapeIdentifier(column.Name));
        ...
        public override IEnumerable<string> CreateTable(ITableSourceInfo newTable, params ColumnDetails[] columns) {
            ...
            bool isTextPK = primaryKey != null && primaryKey.DataType.Type == DBDataType.TEXT;
            ...
            if (isTextPK && dbConfig.CI_AI) {
                statements.Add(
                    CreateAdditionalPrimaryKeyIndexForCIAI(newTable.Name, GetGeneratedPrimaryKeyConstraintNameForTable(newTable), primaryKey));
            }
        ...
        protected override IEnumerable<string> CreatePrimaryKey(ITableSourceInfo existingTable, IPlatformTableSourceColumnInfo column) {
            IList<string> statements = new List<string>();
            var dbConfig = DatabaseServices.DatabaseConfiguration as RuntimeDatabaseConfiguration;
            if (dbConfig == null) {
                return null;
            ...
            statements.AddRange(base.CreatePrimaryKey(existingTable, column));            

            if (column.DataType.Type == DBDataType.TEXT && dbConfig.CI_AI) {
                ...
                statements.Add(CreateAdditionalPrimaryKeyIndexForCIAI(existingTable.Name, GetGeneratedPrimaryKeyConstraintNameForTable(existingTable),
                    column));
            }
            return statements;
        }
