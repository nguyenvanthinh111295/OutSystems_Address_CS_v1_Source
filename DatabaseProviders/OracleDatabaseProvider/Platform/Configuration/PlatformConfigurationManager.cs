...
namespace OutSystems.HubEdition.DatabaseProvider.Oracle.Platform.Configuration {
    public class PlatformConfigurationManager : BasePlatformConfigurationManager {
        ...
        public override FileStream StreamForScriptFile {
            get {
                string path = Path.Combine(Script_Path, "runtimemodel_oracle.sql");
        ...
        private static readonly Regex _osLogTablespaceRegex = new Regex(@"TABLESPACE\s*""OSLOG""",
            RegexOptions.Singleline | RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex _osAdminUserRegex = new Regex(@"HubAdmin",
            RegexOptions.Singleline | RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex _osLogUserRegex = new Regex(@"HubLog",
            RegexOptions.Singleline | RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase);
        ...
        public override string ProcessStatement(string statement) {
            ...
            statement = _osLogTablespaceRegex.Replace(statement, @"TABLESPACE """ + config.LogTablespace + @"""");
            statement = _osAdminUserRegex.Replace(statement, config.AdminUser);
            statement = _osLogUserRegex.Replace(statement, config.LogUser);
            ...
            if (((RuntimeDatabaseConfiguration)uiConfiguration.RuntimeDatabaseConfiguration(Source.Services, User.Admin)).CI_AI &&
        ...
        public override IEnumerable<Block> ExtraPlatformStatements(Version currentModelVersion) {
            ...
            var adminRuntimeConf = (RuntimeDatabaseConfiguration)uiConfiguration.RuntimeDatabaseConfiguration(Source.Services, User.Admin);
        ...
        public override bool TestAdminConnection(out string friendlyMessage, Source source) {
            ...
            if (success) {
                success = TestConnectionUtils.CheckTablespaceExists(uiConfiguration.RuntimeDatabaseConfiguration(source, User.Admin) as RuntimeDatabaseConfiguration,
                    User.Admin, out friendlyMessage);
            }
            return success;
        }
        ...
        public override bool TestRuntimeConnection(out string friendlyMessage, Source source) {
            ...
            if (success) {
                success = TestConnectionUtils.CheckTablespaceExists(uiConfiguration.RuntimeDatabaseConfiguration(source, User.Runtime) as RuntimeDatabaseConfiguration,
                    User.Runtime, out friendlyMessage);
            }
            return success;
        }

        public override bool TestLogConnection(out string friendlyMessage, Source source) {
            bool success = base.TestLogConnection(out friendlyMessage, source);
            if (success) {
                success = TestConnectionUtils.CheckTablespaceExists(uiConfiguration.RuntimeDatabaseConfiguration(source, User.Log) as RuntimeDatabaseConfiguration,
                    User.Log, out friendlyMessage);
            }
            return success;
        }
