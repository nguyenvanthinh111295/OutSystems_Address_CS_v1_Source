...
namespace OutSystems.HubEdition.DatabaseProvider.SqlServer.Platform.Configuration {
    ...
    public static class ConfigurationManagerUtils {
        ...
        public static SqlEngineEdition GetSqlEngineEdition(IPlatformDatabaseConfiguration config) {
            return GetSqlEngineEdition(config.RuntimeDatabaseConfiguration(Source.Application, User.Admin));
        }
        ...
        public static void CheckIfAzureDatabaseExists(IRuntimeDatabaseConfiguration config, string catalog) {
            if (!IsDatabaseAzure(config) || DatabaseExists(config, catalog)) {
                ...
                throw new ConfigurationOperationException(string.Format("Database '{0}' does not exist.\nAutomatic database creation for Azure is not supported, please manually create the required database.\r\nAfterwards, click the 'Grant Permissions' button again.", catalog));
            }
        }
        ...
        private static bool CheckElevatedPermissionsForAzure(IRuntimeDatabaseConfiguration config) {
            // Roles and permissions are different from SQL Server to Azure,
            // we can't check for the specific permissions, so we'll check
            // if the user has a loginmanager role. This role allows the user
            // to create logins. We won't need to check for 'dbmanager' for Azure 
            // as databases won't be automatically created.
            string sql =
                @"SELECT 
                    COUNT(*) 
                ...
                WHERE p.name = USER 
                    AND p2.name = 'loginmanager'";

            return Convert.ToInt32(ExecuteScalar(config, sql)) == 1;
        }
        ...
        public static void CheckIfUserHasNecessaryElevatedPermissions(IRuntimeDatabaseConfiguration config) {
            try {
                bool hasAdminPermissions = false;

                if (IsDatabaseAzure(config)) {
                    hasAdminPermissions = CheckElevatedPermissionsForAzure(config);
                } else {
                    hasAdminPermissions = CheckElevatedPermissionsForSQLServer(config);
                }

                if (!hasAdminPermissions) {
                    throw new ConfigurationOperationException(string.Format("User {0} doesn't have admin permissions", config.Username));
                }
            } catch (Exception e) {
        ...
        public static bool NeedsCreate(IRuntimeDatabaseConfiguration confMaster, IRuntimeDatabaseConfiguration conf, string user) {
            // The additional config parameter for master catalog is needed for Azure, 
            // due to limits in cross-catalog queries.
            return !IsDBAdmin(conf, user) && !LoginExists(confMaster, user);
        }
        ...
        public static void GrantUserPermissions(PlatformDatabaseConfiguration platformConfiguration) {

            var config = platformConfiguration.ElevatedRuntimeDatabaseConfiguration();

            var configMaster = ConfigurationToMaster(platformConfiguration);

            var authType = platformConfiguration.AuthenticationMode;

            // Create User

            if (NeedsCreate(configMaster, config, platformConfiguration.AdminUser)) {
                if (authType == AuthenticationType.Windows_Authentication) {
                    AddIntegratedAuthLogin(config, platformConfiguration.AdminUser);
                } else {
                    AddDBAuthLogin(configMaster, platformConfiguration.AdminUser, platformConfiguration.AdminPassword);
                }
            }

            if (AdminNeedsPermissions(config, platformConfiguration.AdminUser)) {
                AddAdminPermissions(config, platformConfiguration.AdminUser);
            }

            if (NeedsCreate(configMaster, config, platformConfiguration.RuntimeUser)) {
                if (authType == AuthenticationType.Windows_Authentication) {
                    AddIntegratedAuthLogin(config, platformConfiguration.RuntimeUser);
                } else {
                    AddDBAuthLogin(configMaster, platformConfiguration.RuntimeUser, platformConfiguration.RuntimePassword);
                }
            }

            if (RuntimeNeedsPermissions(config, platformConfiguration.RuntimeUser, platformConfiguration.AuthenticationMode)) {
                AddRuntimePermissions(config, platformConfiguration.RuntimeUser);
            }

            if (authType == AuthenticationType.Windows_Authentication) {
                AddExtraRuntimePermissionsIntegratedAuth(config, platformConfiguration.RuntimeUser);
            }

            try {
                AddRuntimePermissionsAfterUpgradeScript(config, platformConfiguration.RuntimeUser);
            } catch { }

            if (NeedsCreate(configMaster, config, platformConfiguration.LogUser)) {
                if (authType == AuthenticationType.Windows_Authentication) {
                    AddIntegratedAuthLogin(config, platformConfiguration.LogUser);
                } else {
                    AddDBAuthLogin(configMaster, platformConfiguration.LogUser, platformConfiguration.LogPassword);
                }
            }

            if (LogNeedsPermissions(configMaster, platformConfiguration.LogUser, platformConfiguration.AuthenticationMode)) {
                AddLogPermissions(config, platformConfiguration.LogUser);
            }

            if (authType == AuthenticationType.Windows_Authentication) {
                AddExtraLogPermissionsIntegratedAuth(config, platformConfiguration.LogUser);
            }
        }

        private static bool NeedsPermissions(List<string> rolesAvailable, string masterRole, List<string> rolesNeed) {
            return rolesAvailable.Contains(masterRole) || rolesNeed.All(rolesAvailable.Contains);
        }
        ...
        private static List<string> GetUserRolesOrOwner(IRuntimeDatabaseConfiguration config, string userName) {
            if (IsDBAdmin(config, userName)) {
                return new List<string> { "db_owner" };
            }
            return GetUserRoles(config, userName);
        }

        private static bool LogNeedsPermissions(IRuntimeDatabaseConfiguration conf, string username, AuthenticationType auth) {
            try {
                var logRoles = new List<string> { "db_datareader", "HubLogRole" };
                if (auth == AuthenticationType.Windows_Authentication) {
                    logRoles.Add("db_datawriter");
                }
                return NeedsPermissions(GetUserRolesOrOwner(conf, username), "db_owner", logRoles);
            } catch {
                return true;
            }
        }

        private static bool AdminNeedsPermissions(IRuntimeDatabaseConfiguration conf, string username) {
            try {
                var localAdminRoles = new List<string> { "db_accessadmin", "db_securityadmin", "db_ddladmin", "db_datareader" };
                return NeedsPermissions(GetUserRolesOrOwner(conf, username), "db_owner", localAdminRoles);
            } catch {
                return true;
            }
        }

        private static bool RuntimeNeedsPermissions(IRuntimeDatabaseConfiguration conf, string username, AuthenticationType auth) {
            try {
                var runtimeRoles = new List<string> { "db_datareader", "db_datawriter" };
                if (auth == AuthenticationType.Windows_Authentication) {
                    runtimeRoles.Add("HubLogRole");
                }
                return NeedsPermissions(GetUserRolesOrOwner(conf, username), "db_owner", runtimeRoles);
            } catch {
                return true;
            }
        }

        public static void AddRuntimePermissionsAfterUpgradeScript(IRuntimeDatabaseConfiguration config, string username) {
            foreach (string statement in GetGrantRuntimePermissionsStatements(username)) {
                ExecuteNonQuery(config, statement);
            }
        }

        public static IEnumerable<string> GetGrantRuntimePermissionsStatements(string username) {
            yield return "grant execute on dbo.Dequeue_ossys_Event to [" + username + "]";
            yield return "grant execute on dbo.Dequeue_ossys_Email_Status to [" + username + "]";
            yield return "grant execute on dbo.Dequeue_ossys_Email_Status_v2 to [" + username + "]";
            yield return "grant execute on dbo.CheckRole to [" + username + "]";
            yield return "grant execute on dbo.Dequeue_ossys_BPM_Event to [" + username + "]";
            yield return "grant execute on dbo.Dequeue_ossys_Event_Beta to [" + username + "]";
        }
        ...
        private static int AddDBAuthLogin(IRuntimeDatabaseConfiguration config, string username, string password) {
            try {
        ...
        private static int AddIntegratedAuthLogin(IRuntimeDatabaseConfiguration config, string username) {
            try {
        ...
        private static readonly string[] adminRoles = { "db_accessadmin", "db_securityadmin", "db_ddladmin", "db_datareader", "db_datawriter" };

        private static int AddAdminPermissions(IRuntimeDatabaseConfiguration config, string username) {
            try {
                ...
                foreach (string role in adminRoles) {
                    sql += "\n" + SQLGetStatement_AddRoleMember(config, role, username);
                }
        ...
        private static int AddRuntimePermissions(IRuntimeDatabaseConfiguration config, string username) {
            try {
                string sql = @"if not exists (select * from sys.database_principals where name = N'" + username + @"' AND type = '" + SQLGetUserType(username) + @"')
                    " + SQLGetStatement_CreateUser(config, username, username) + @"
        ...
        private static int AddLogPermissions(IRuntimeDatabaseConfiguration config, string username) {
            try {
                ...
                string sql = @"if not exists (select * from sys.database_principals where name = N'" + username + @"' AND type = '" + SQLGetUserType(username) + @"')
                        ...
                        " + SQLGetStatement_DropRoleMember(config, "db_ddladmin", "HubLogRole") + @"
                        " + SQLGetStatement_AddRoleMember(config, "db_datareader", username) + @"
        ...
        private static int AddExtraLogPermissionsIntegratedAuth(IRuntimeDatabaseConfiguration config, string username) {
            try {
                string sql = SQLGetStatement_AddRoleMember(config, "db_datawriter", username);
                return ExecuteNonQuery(config, sql);
            } catch (Exception e) {
                throw new ConfigurationOperationException("Adding permissions to " + username + ": " + e.Message, e);
            }
        ...
        private static int AddExtraRuntimePermissionsIntegratedAuth(IRuntimeDatabaseConfiguration config, string username) {
            try {
        ...
        private static bool IsDBAdmin(IRuntimeDatabaseConfiguration config, string login) {
            try {
        ...
        private static string GetHubServerVersion(IRuntimeDatabaseConfiguration config) {
            string sql = "SELECT Val FROM ossys_Parameter WHERE Name = 'version'";
        ...
        private static bool CheckBuild(string version1) {
        ...
        public static void CheckRoles(IPlatformDatabaseConfiguration platformConfig, out string friendlyMessage, Source source, User user, string username) {
            try {
                ...
                var config = platformConfig.RuntimeDatabaseConfiguration(source, user);
                try {
                    if (IsDBAdmin(config, username)) {
                        owner = true;
                    } else {
                    ...
                    switch (user) {
                        ...
                        case User.Runtime:
                            if (!owner) {
                                ...
                                if (!hasLogRole && platformConfig.AuthenticationMode == AuthenticationType.Windows_Authentication) {
                        ...
                        case User.Log:
                            if (!owner) {
                                ...
                                if (!canWrite && platformConfig.AuthenticationMode == AuthenticationType.Windows_Authentication) {
                                    friendlyMessage = "Database connection was successful.\r\n\r\nHowever the user does not have the following permission(s):\r\n\r\nUser " + username + " doesn't have write permissions.";
                                    return;
                                }
                                if (!hasLogRole) {
                    ...
                    object obj = GetHubServerVersion(config);
                    if (obj != null) {
        ...
        private static Version InnerGetExecutingAssemblyVersion() {
