    ...
    public class PlatformSessionService : IPlatformSessionService {
        ...
        public void UnlockSessionBySubstring(int position, string subString) {
            try {
            	using (var con = DatabaseServices.TransactionService.CreateConnection()) {
	                var e = DatabaseServices.ExecutionService;
	                var sqlUnlockSMS = "dbo.SMSUnlockSessionsWithSubstring";
	                using (var cmd = e.CreateCommand(con, sqlUnlockSMS)) {
                        cmd.CommandTimeout = queryTimeout;
	                    e.CreateParameter(cmd, "@position", DbType.Int32, position);
	                    e.CreateParameter(cmd, "@subString", DbType.String, subString);
	                    cmd.ExecuteNonQuery();
	                }
	            }
            } catch (Exception e) {
        ...
        public void InsertInitialized(string sessionId, byte[] data, System.TimeSpan timeout) {
            // only for java
            throw new System.NotImplementedException();
        }
