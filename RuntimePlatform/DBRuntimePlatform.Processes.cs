...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public partial class DBRuntimePlatform {
        ...
        public void DeleteLightEvent(Transaction trans, int id) {
            DeleteEvent(trans, id, LightEventQueueTableName);
        }

        private void DeleteEvent(Transaction trans, int id, string tableName) {
        ...
        protected const string EventDequeueProcedureName = "DEQUEUE_OSSYS_EVENT";
        protected const string LightEventDequeueProcedureName = "DEQUEUE_OSSYS_BPM_EVENT";
        protected const string EventDequeueProcedureNameImproved = "DEQUEUE_OSSYS_EVENT_BETA";
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void RequeueLightEvent(Transaction trans, int id, bool hasError) {
            RequeueEvent(trans, id, hasError, LightEventQueueTableName);
        }

        private void RequeueEvent(Transaction trans, int id, bool hasError, string tableName) {
...
#if JAVA
            return "CALL {0}(@BATCHSIZE, @IP, @NULLDATE, @ACTIVITYID)".F(EventDequeueProcedureName);
#else
            return EventDequeueProcedureName;
#endif
        }

        protected virtual string GetDequeueLightEventSQL() {
#if JAVA
            return "CALL {0}(@BATCHSIZE, @IP, @NULLDATE, @ACTIVITYID)".F(LightEventDequeueProcedureName);
#else
            return LightEventDequeueProcedureName;
#endif
        }
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public IDataReader DequeueEvent(Transaction trans, bool isLightEvent, int batch, string ip, int activityId) {
            if (isLightEvent) {
                return this.DequeueLightEvent(trans, batch, ip, activityId);
            } else {
                return this.DequeueNormalEvent(trans, batch, ip, activityId);
            }
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        public IDataReader DequeueNormalEvent(Transaction trans, int batch, string ip, int activityId) {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public IDataReader DequeueLightEvent(Transaction trans, int batch, string ip, int activityId) {
