...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public class DebugSession {
        ...
        private const string OutSystemsAppUserAgent = "OutSystemsApp";
        ...
        private string GetRequestRealIP(HttpRequest request) {
        ...
        public bool RequestAppliesToSession() {
            ...
            // if the setting considering every ip is on or the request comes from an outsystems mobile app (native shell)
            if (IsNewRuntimeApplication() && (Settings.GetBool(Settings.Configs.AllowDebuggingMobileAppsFromAnyIP) || (request.UserAgent ?? "").Contains(OutSystemsAppUserAgent))) { 
                return true;
            }

            // - local requests apply to all sessions
            // - remote requests apply only to clients with the same ip
            string requestIp = GetRequestRealIP(request);
