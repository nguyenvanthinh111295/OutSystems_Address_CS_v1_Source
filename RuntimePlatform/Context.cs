...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public class SessionInfo {
        ...
        public SessionInfo() {
            ...
            NewRuntimeLoginInfo = null;
        }

        public SessionInfo(MobileLoginConfiguration mobileLoginConfiguration) {
            _variables = new SessionHashtable();
            NewRuntimeLoginInfo = new LoginInfo(mobileLoginConfiguration);
        }
        ...
        private Dictionary<int, string> ReadAllUserRoles() {
            ...
            using (Transaction tran = DatabaseAccess.ForSystemDatabase.GetReadOnlyTransaction()) {
                using (var reader = DBRuntimePlatform.Instance.GetAllUserRoles(tran, UserId, TenantId)) {
                    ...
                    while (reader.Read()) {
                        var compoundKey = new GlobalObjectKey(ObjectKey.Parse(DataReaderUtils.SafeGet<string>(reader[2])), ObjectKey.Parse(DataReaderUtils.SafeGet<string>(reader[1])));
                        allUserRoles.Add(DataReaderUtils.SafeGet<int>(reader[0]), compoundKey.ToString(ObjectKey.ShortFormat));
                    }
                    return allUserRoles;
                }
            }
    ...
    public class HeContext { // He prefix to avoid the need of prefixing with namespace (conflicts)
        ...
        public string CurrentLocale {
            get {
                ...
                } else if (currentCulture != null && _context.Session != null && _context.Session["Locale"] == null) {
                    // RPD-1478 - The session can be reseted cleared. However inThe session can be reset cleared. However, 
                    // in this case, we are not cleaning the local value of currentCulture. This can produce error behavior 
                    // because it may assume the value of current local haven't changed when it did.
                    currentCulture = null;
                }
