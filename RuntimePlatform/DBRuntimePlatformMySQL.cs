...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    internal class DBRuntimePlatformMySQL : DBRuntimePlatform {
        ...
        public override void UpdateSerialNumber(Transaction tran, string previousSerial, string newSerial) {
            string sql = "UPDATE ossys_parameter p1 set p1.host_serial = @currentSerial where upper(p1.host_serial) = @previousSerial AND NOT EXISTS (SELECT 1 FROM (select * from ossys_parameter) p2 WHERE p1.name=p2.name AND  upper(p2.host_serial) = upper(@currentSerial))";
